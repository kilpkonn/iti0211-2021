...
reisi(X, Y, Path, Cost) :- laevaga(X, Z, C1),
  Path = mine(X, Z, laevaga, SubPath),
  Cost is C1 + C2,
  reisi(Z, Y, SubPath, C2).
reisi(X, Y, Path, Cost) :- bussiga(X, Z, C1),
  Path = mine(X, Z, bussiga, SubPath),
  Cost is C1 + C2,
  reisi(Z, Y, SubPath, C2).
reisi(X, Y, Path, Cost) :- rongiga(X, Z, C1),
  Path = mine(X, Z, rongiga, SubPath),
  Cost is C1 + C2,
  reisi(Z, Y, SubPath, C2).
reisi(X, Y, Path, Cost) :- lennukiga(X, Z, C1),
  Path = mine(X, Z, lennukiga, SubPath),
  Cost is C1 + C2,
  reisi(Z, Y, SubPath, C2).
